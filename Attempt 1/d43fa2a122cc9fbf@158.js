...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["query.csv",new URL("/Users/rebeccatast/Downloads/radial-stacked-bar-chart/query.csv",import.meta.url)]]);
...
d3.csvParse(await FileAttachment("query.csv").text(), (d, _, columns) => {
  let total = 0;
...
d3.arc()
    .innerRadius(d => y(d[0]))
    .outerRadius(d => y(d[1]))
    .startAngle(d => x(d.data.locationSource))
    .endAngle(d => x(d.data.locationSource) + x.bandwidth())
    .padAngle(0.01)
    .padRadius(innerRadius)
)});
  main.variable(observer("x")).define("x", ["d3","data"], function(d3,data){return(
d3.scaleBand()
    .domain(data.map(d => d.locationSource))
    .range([0, 2 * Math.PI])
    .align(0)
)});
...
g => g
    ...
    .call(g => g.selectAll("g")
      ...
      .join("g")
        .attr("transform", d => `
          rotate(${((x(d.locationSource) + x.bandwidth() / 2) * 180 / Math.PI - 90)})
        ...
        .call(g => g.append("text")
            .attr("transform", d => (x(d.locationSource) + x.bandwidth() / 2 + Math.PI / 2) % (2 * Math.PI) < Math.PI
